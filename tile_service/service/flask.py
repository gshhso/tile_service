...
class TileServiceConfig:
	...
	def __init__(
		self,
		data_root: Pathlike,
		index_file: Optional[Pathlike] = "index.pkl",
		luotu_file: Optional[Pathlike] = "luotu.geojson",
		tile_size: int = 256,
		cache_size: int = 128,
...
class TileService:
	...
	def _load_spatial_index(self) -> tuple[GeoDataFrame, SpatialIndex]:
		...
		# 检查索引文件是否存在
		if not (self.config.index_file and self.config.index_file.exists() and 
				self.config.luotu_file and self.config.luotu_file.exists()):
			# 自动构建索引
			image_list = list(self.config.data_root.rglob("**/*.tif"))
			if not image_list:
				raise FileNotFoundError(f"在 {self.config.data_root} 中未找到任何.tif文件")
			...
			self.build_index(
				...
				self.config.luotu_file.name if self.config.luotu_file else "luotu.geojson",
	...
	def _xyz_to_tile_data(self, x: int, y: int, z: int) -> np.ndarray:
		...
		Returns:
			瓦片数据的numpy数组
		"""
		from ..tile import convert_xyz_to_bbox, filter_intersect_image, load_dataarray, get_tiles
		...
		if not intersect_images:
			# 返回空瓦片
			return np.zeros((self.config.tile_size, self.config.tile_size, 3), dtype=np.uint8)
